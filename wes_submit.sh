...
# Config file - use argument or default to config.yaml in script directory
...
# Check that wes_pipline.sh exists
WES_PIPELINE="${SCRIPT_DIR}/wes_pipline.sh"
if [ ! -f "$WES_PIPELINE" ]; then
    echo -e "${RED}ERROR: wes_pipline.sh not found in: $SCRIPT_DIR${NC}"
    exit 1
fi
